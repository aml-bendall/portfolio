...
//Set date to today for SQL queries
DecimalFormat df = new DecimalFormat("#.00");
DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd");
LocalDate localDate = LocalDate.now();

try {
	//Connect to the CutSheet table to gather a count of all users who have cut fabric today.
	...
	sql="select startUser From CutSheet where pickTime>=dateadd(day, datediff(day, 0, getdate()), 0) + '0:00:00' and startUser is not null group by startUser";
		...
		try {
			//Connect to the database to get the name of any user who has aa cut currently in progress.
			java.sql.Connection indexCon =  db.dbConnect("jdbc:sqlserver://FIBRE-SQL;databaseName=FibreApps");
		...
		try {
			//Connect to the SQL database to get user specific data and save it to the Cuts array.
			...
			sql ="SELECT startUser, SUM(Total_Time) as Total_Time "+ 
					  "From ( "+ 
								...
								"from "+
									"CutSheet where inProgress=2 and endCutTime>'"+localDate+" 00:00:00' and endCutTime<'"+localDate+" 23:59:59' "+
								"UNION ALL "+
								"select "+
									"startUser,DATEDIFF(MINUTE, startCutTime, GetDate()) as Total_Time from CutSheet where inProgress=1  and startCutTime>'"+localDate+" 00:00:00' and startCutTime<'"+localDate+" 23:59:59' "+
								") as tbl "+
						  "Group by "+
									"startUser";
...
<%
		try {
			...
			sql="select endUser, number, quant from CutSheet where pickTime>=dateadd(day, datediff(day, 0, getdate()), 0) + '0:00:00' and endUser is not null";
	...
	<td>
		...
		try {
			...
			sql="select endUser,count(endUser) as Cuts from CutSheet where inprogress=2 and endCutTime>=dateadd(day, datediff(day, 0, getdate()), 0) + '0:00:00' group by endUser";
...
totalTime=totalTime+Double.valueOf(Cuts[i][3]);
	...
	<td>
		...
		<%	
		//Use the data in the array to determine the average time of when orders came in vs when they were completed.
		for(int i = 0; i < Cuts.length; i++)
		{
			if(Cuts[i][0]!= null) {
				%>
	...
	<td>
		...
		try {
			...
			sql="select startUser,AVG(IIF(picktime<dateadd(day, datediff(day, 0, pickTime), 0) + '8:00',DATEDIFF(MINUTE, dateadd(day, datediff(day, 0, pickTime), 0) + '8:00', startCutTime), DATEDIFF(MINUTE, picktime, startCutTime))) as Prod_Queue from CutSheet where pickTime>=dateadd(day, datediff(day, 0, getdate()), 0) + '0:00:00' and Prepack!=1 and inProgress!=0 group by startUser";
